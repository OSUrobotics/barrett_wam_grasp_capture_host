...
class Recapturer:
    def __init__(self):
        ...
        self.msg_data['realsense1'] = None
        self.msg_data['realsense2'] = None
        ...
        self.subscribers.append(rospy.Subscriber("/camera1/depth/points/world_frame", PointCloud2, self.generic_capture, callback_args='realsense1'))
        self.subscribers.append(rospy.Subscriber("/camera2/depth/points/world_frame", PointCloud2, self.generic_capture, callback_args='realsense2'))
    ...
    def set_msg(self, k, v):
        try:
            self.msg_data[k] = v
        except KeyError:
            rospy.logerr("Cannot enter data value. Missing key in Recapturer.set_msg: %s" % str(k))
...
# Returns a dictionary of the timestamps of extreme ranges and optimals
...
def repub_data_stamp(grasp_stamp, video_bag, cam_info_spammer, relevant_data):
    max_num_pub_msgs = 10
    pub_next_msgs = -1
    ...
    for topic, msg, t in video_bag.read_messages(start_time=(grasp_stamp - lead_time), end_time=(grasp_stamp + play_time)):
        ...
        if pub_next_msgs > 0 and "color" in topic:
        ...
        elif pub_next_msgs > 0 and "depth" in topic and "kinect" in topic:
            if "compressed" in topic:
            ...
            continue
        elif 'camera1' in topic and 'world_frame' in topic:
            relevant_data.set_msg('realsense2', msg)
            continue
        elif 'camera2' in topic and 'world_frame' in topic:
            relevant_data.set_msg('realsense1', msg)
            continue
    ...
    for (data_dir_path, obj_num, sub_num) in data_dirs:
        ...
        for grasp_set_num in sorted(relevant_grasp_timestamps.keys()):
            ...
            for idx, grasp_tuple in enumerate(grasp_set):
                ...
                grasp_cloud = None
                while True:
                    data_retrievable = repub_data_stamp(grasp_stamp, video_bag, cam_info_spammer, relevant_data)
                    ...
                    try:
                        ...
                        rs1_cloud = relevant_data.get_data('realsense1')
                        rs2_cloud = relevant_data.get_data('realsense2')
                        break
                    except:
                ...
                try:
                    ...
                    out_cloud.header.frame_id = 'ar_marker_4'
                ...
                grasp.realsense_feed1 = rs1_cloud
                grasp.realsense_feed2 = rs2_cloud
