...
#        self.gui.register_button_cb("_resume_trial", self.resume_trial)
...
#        rospy.logerr("NOT RESTORING THE STATE OF THE PROGRAM.")
#        #self.cur_grasp_data.disallow_annotations()
#        #with rosbag.Bag(cur_grasp_data.get_logging_dir() + '', 'r') as b
...
class GraspData:
    ...
    def make_data_dir(self, allow_append):
        ...
        if os.path.exists(self.instance_dir) and not allow_append:
            # Path exists, but appending is not allowed
            rospy.logwarn("Grasping directory already exists. Appending unique id.")
            self.gui.show_error("Grasping directory already exists. Skipping")
    ...
    def start_robot_grasp_annotations(self):
        ...
        self.annotation_id = self.bag_manager.start_recording(self.robot_annotation_path, [self.annotations_topic], self.append_bag)[0]
        self.enable_annotations()

    def start_human_grasp_annotations(self):
        rospy.loginfo("Starting human grasp_annotations.")
        self.annotation_id = self.bag_manager.start_recording(self.human_annotation_path, [self.annotations_topic], self.append_bag)[0]
        self.enable_annotations()

    def enable_annotations(self):
        enable_gui_elements(self.gui, self.starting_annotation_elements)

    def disallow_annotations(self):
        self.allow_annotations = False
    def allow_annotations(self):
        self.allow_annotations = True
